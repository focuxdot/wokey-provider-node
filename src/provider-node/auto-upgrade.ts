...
export class AutoUpgradeController {
  ...
  async handleUpgradeAvailable(message: PlatformUpgradeAvailable): Promise<void> {
    ...
    try {
      await this.executeUpgrade(message.version, currentVersion, expectedHash);
    } catch (err) {
  ...
  private async executeUpgrade(targetVersion: string, currentVersion: string, expectedHash: string): Promise<void> {
