...
- Keep PKCE/device state and vendor token responses on Platform; remove the
  Provider Node native OAuth and Cursor polling implementations.

## [0.1.81]

### Changed
- Keep credential-channel reconnect and liveness diagnostics in the local
  status surface instead of duplicating Platform-owned channel health in
  heartbeat messages.
