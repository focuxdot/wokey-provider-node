...
6. 确认成功：在本地节点管理页面就能看到已授权的账号；在网站的 Provider 页面也能看到。（无界面的服务器可改用 `wokey-node status`。）
...
对于 xAI OAuth 凭证，节点仅精确放行 `cli-chat-proxy.grok.com`，供平台经供应方节点调用
`/v1/user?include=subscription` 执行 Grok 官方订阅查询；这不会放行任意
`*.grok.com` 域名。
